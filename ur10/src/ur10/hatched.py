...
def _build_hatch(
    ...
    try:
        ...
        if circular:
            ...
            # This will be a list of lists, one list of lines per level
            ...
            for angle in hatch_angle:
                ...
                # Only divide if angle is not 0 or 180 (horizontal)
                ...
                for i in range(n_levels):
                    all_lines_by_level[i].extend(lines_for_this_angle[i])

            # `lines` is now `all_lines_by_level`, which contains all angles
            lines = all_lines_by_level
            # --- END MODIFICATION ---
...
def hatch(
    ...
    :param hatch_angle: angle (or list of angles) that defines hatching inclination (degrees)
