...
The app is a single **Dear PyGui** window with tabs: Flow Imager, Hatched, Dither, Trace, Text, Roads, UR10 Control, and Instructions.
...
- **centerline / trace** ([centerline_vectorizer.py](src/ur10/vectorizers/centerline_vectorizer.py)) — recreates a raster **line drawing** as single-stroke centerlines: resample to a target long-edge (the *Resolution* control **up**scales small sources as well as downscaling big ones — small/bold text needs the extra pixels to skeletonize into clean letters) → binarize (Otsu/manual + invert) → `skimage.morphology.skeletonize` (collapses each stroke's *thickness* to a 1-px centerline, so a thick or double-drawn outline becomes **one** pen line) → prune spurs (erase short "whisker" branches that dead-end at a junction; kept modest because short *real* strokes like letter bars look like spurs) → trace the skeleton's pixel graph into polylines (degree-1 endpoints / degree≥3 junctions are nodes, degree-2 runs are edges walked once; a second pass seeds pure loops like the lens rings that have no node) → drop specks (min length) → smooth each line (resample to arc-length-spaced, locally-averaged anchors → cubic B-spline, to shed the 1-px skeleton *staircase* wobble; a bounded anchor count is used deliberately, since FITPACK's auto-knot smoothing spline explodes its knot count — and stalls — on jittery data) → simplify (shapely RDP). Plain-thread + cooperative `stop_event` (the Hatched pattern). Its [tab](src/ur10/app/tab_trace.py) subclasses `VectorizerTab`, so Optimize/Save/preview are inherited unchanged. Note: filled/bold shapes (logo text) trace as single-stroke "stick" letters — legible, but not the outlined logo; an outline-trace mode would be a separate path.
...
1. **Extract** (`pipeline.extract_road`): two `lat, lon` points → `osm` downloads the local road graph and routes between them → `geometry` projects to UTM, trims to the exact endpoints, simplifies → `ExtractedRoad`. Only this stage needs the network; **osmnx is imported lazily** inside the extract worker so startup doesn't pay for it. Runs off-thread via the tab's `EventBridge` (`-ROADS_DONE-`), like a vectorizer (plain daemon thread — no process kill). After routing, the worker also samples an **elevation profile** (`road_outline_extracter/elevation.py`, the free key-less Open-Topo-Data API over stdlib `urllib`) to fill `ExtractedRoad.elevation_loss_m` (total descent) and `max_grade_pct` (steepest sustained downhill). This is **best-effort** — any network/API failure is logged and the road is used without it — and it needs no elevation-specific dependency.
2. **Plot** (`layout.plan`): `ExtractedRoad.line_utm` + `PlotConfig` → rotate → fit-to-canvas (rotation-aware) → center + nudge → `stroke_count` parallel passes (bolding) → `svg.render` emits a **millimeter** SVG (`<path>` M/L, `viewBox` in mm). Pure and fast, so the preview replots live on each control edit. When a label is shown, its footprint is passed to `plan` as obstacle rectangles and the road is fit into the **largest label-free rectangle** inside the margin instead of the whole canvas, so the road never overlaps the text. The gap kept around the label is adjustable (`LabelConfig.clearance_mm`, the tab's *Road gap (mm)*).

**Optional metadata label:** the tab can annotate the drawing in **single-stroke Hershey text** (same `FONTS` as the Text tab). The nickname is a title pinned to the **canvas top-center** (+2 mm); the rest — real name / road length / descent (m/ft) / max grade (%) / start+finish coords (one row) / date — form a block anchored at the chosen Position (with x/y nudge). Each field is toggleable. `road_outline_extracter/label.py` renders the chosen fields (`LabelConfig`) to `LineString`s in the *same y-up mm space* as the road and the tab merges them into `result.strokes`, so the label flows through the preview, the saved SVG, and the robot identically — it is drawn, not a UI overlay. The label is placed **first** (it doesn't depend on the road's shape); `label.obstacle_boxes` reports the title + body-block rectangles, which the tab hands to `layout.plan` so the road is fit clear of them (see the Plot stage above). Glyph geometry is cached (`build_rows` / `render_rows` / `place_rows` split) so dragging position/nudge stays smooth. `LabelConfig` persists per-run as `roads/<slug>/label.json`; Real name / Nickname / Date inputs live in the Saved Runs section and persist in `meta.json`.

A separate toggle adds a minimal **north compass** (`road_outline_extracter/compass.py`) pinned to the bottom-right corner — a symmetric diamond needle + "N" (no ring) whose needle points to where north lands after the plot rotation (`(-sin θ, cos θ)` in y-up), so the viewer can orient the drawing however the road was rotated to fit. It is pure geometry (no font) and merges into `result.strokes` like the label; its toggle + radius live in `LabelConfig` (`label.json`).
